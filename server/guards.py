...
def validate_pdf_bytes(data: bytes, filename: str) -> Dict[str, Any]:
    ...
    if found_active:
        logger.warning(f"Rejected '{filename}': active content {found_active}")
        raise ValidationError(
...
def _safe_page_count(data: bytes, filename: str) -> int:
    ...
    try:
        ...
        logger.warning(f"Rejected '{filename}': unreadable ({exc})")
...
async def check_intent(
    ...
    try:
        ...
        if allowed is None:
            logger.warning(f"Intent guard returned no verdict: {verdict}")
            return False, "classifier gave no verdict"

        return bool(allowed), reason

    except Exception as exc:
        logger.warning(f"Intent guard unavailable, falling through: {exc}")
