...
def client_ip(request: Request) -> str:
    ...
    if forwarded:
        # Scrubbed at the boundary: this is a raw header, it is only ever used
        # for rate-limit keys and log lines, and sanitising it here means no
        # caller has to remember to.
        return guards.scrub(forwarded.split(",")[0].strip(), limit=64)
...
async def require_session(authorization: Optional[str] = Header(default=None)) -> str:
    ...
    The id is scrubbed on the way out even though the token it came from was
    HMAC-verified. It is threaded through nearly every log line in this module,
    and one sanitising point beats trusting a dozen call sites to remember.
    """
    token = None
    ...
    try:
        return guards.scrub(guards.verify_session_token(token), limit=64)
...
async def root() -> Dict[str, Any]:
    """Signpost for anyone who opens the bare service URL.

    Every other route lives under /api, so without this the host's own root
    request gets a bare 404 that reads like a broken deploy. Static and
    unauthenticated on purpose: it names where the UI is and nothing else.
    """
    return {
...
async def upload(
    ...
    # See the note in _stream_answer: Depends() injection is opaque to static
    # analysis, so the barrier has to be spelled out in the handler itself.
    sid = session_id.replace("\r", " ").replace("\n", " ")
    logger.info(
        f"Session {sid} uploading {guards.scrub(filename)} "
        f"({info['page_count']}p) as job {job.job_id}"
    )
...
async def _stream_query(
    ...
    # Spelled out rather than routed through guards.scrub: FastAPI injects
    # session_id through Depends(), which static analysis does not trace, so it
    # treats this parameter as raw request input no matter what the dependency
    # already did to it. Literal CR/LF replacement in the same function is the
    # form it recognises as a barrier.
    sid = session_id.replace("\r", " ").replace("\n", " ")
    ...
    try:
        ...
        if pattern:
            # The pattern is one of our own constants, and !r would escape the
            # newlines anyway, but dataflow carries the taint from `question`
            # through the call and repr is not treated as a barrier.
            safe_pattern = str(pattern).replace("\r", " ").replace("\n", " ")
            logger.info(f"Session {sid} query blocked by pattern {safe_pattern!r}")
            yield sse({
                "type": "refusal",
        ...
        logger.info(f"Session {sid} disconnected mid-stream")
        raise
    except Exception as exc:
        logger.error(
            f"Query failed for session {sid}: {guards.scrub(exc)}",
            exc_info=True,
        )
...
async def _ingest(session_id: str, job: IngestJob, data: bytes, filename: str) -> None:
    ...
    try:
        ...
        logger.info(f"Job {job.job_id} ready: {guards.scrub(filename)}, {len(chunks)} chunks")

    except asyncio.TimeoutError:
        job.status = "failed"
        job.error = f"Parsing took longer than {config.PARSE_TIMEOUT_SECONDS}s and was stopped."
        job.finished_at = time.time()
        logger.warning(f"Job {job.job_id} timed out on {guards.scrub(filename)}")
    except Exception as exc:
        job.status = "failed"
        job.error = "This document could not be processed."
        job.finished_at = time.time()
        logger.error(
            f"Job {job.job_id} failed on {guards.scrub(filename)}: {guards.scrub(exc)}",
            exc_info=True,
        )
    finally:
        if tmp_path:
