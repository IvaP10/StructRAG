...
async def create_session(body: SessionRequest, request: Request) -> Dict[str, Any]:
    ...
    if not guards.check_invite_code(body.invite_code):
        logger.warning(f"Invalid invite code from {guards.scrub(ip)}")
    ...
    logger.info(f"Session {session_id} opened for {guards.scrub(ip)}")
...
async def upload(
    ...
    logger.info(
        f"Session {session_id} uploading {guards.scrub(filename)} "
