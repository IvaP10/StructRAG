# StructRAG API image — targets Hugging Face Spaces (Docker SDK), but it is a
# plain uvicorn container and runs anywhere.
#
...
# Run as an unprivileged user. Hugging Face Spaces runs containers as uid 1000,
# so matching it keeps the writable paths below actually writable.
...
# disabled Space still reports healthy rather than crashed.
