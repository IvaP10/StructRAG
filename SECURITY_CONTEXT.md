...
| API (`server/`, `core/`, root modules) | Hugging Face Space (Docker) | **Yes** | Anyone with the invite code |
...
4. **Integrity of the container.** Remote code execution in the Space would
   expose the key, so it collapses into (1).
...
- Environment variables and Space secrets.
...
- Rate-limit and session state are in-process, so a Space restart clears them.
